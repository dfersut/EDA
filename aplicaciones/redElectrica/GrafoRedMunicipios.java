...
public class GrafoRedMunicipios {
    ...
    public double crearAdyKruskal() {      
        ...
        // COMPLETAR
        double coste = 0.0;
        //ListaConPI<Municipio> municipios = new LEGListaConPI<Municipio>();
        adyKruskal = new TablaHash<Municipio, ListaConPI<Municipio>>(MAX_MUNICIPIOS);
        for(Arista arista : aristas){
            int o = arista.getOrigen(); int d = arista.getDestino(); double p = arista.getPeso();
            Municipio municipioOrigen = verticesAMunicipios.recuperar(o);
            Municipio municipioDestino = verticesAMunicipios.recuperar(d);
            coste += p;
            ListaConPI<Municipio> municipios = adyKruskal.recuperar(municipioOrigen);
            if (municipios == null){
                municipios = new LEGListaConPI<Municipio>();
                adyKruskal.insertar(municipioOrigen, municipios);
            }
            municipios.insertar(municipioDestino);
        }
