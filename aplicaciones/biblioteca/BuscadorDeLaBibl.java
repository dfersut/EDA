...
public class BuscadorDeLaBibl {
    ...
    protected TablaHash<Termino, ListaConPI<BuscadorDeLaBibl.Posting>> index;
    ...
    public ListaConPI<Termino> hapax() {
        ...
        int indexTalla = index.talla();
        
        for(c.inicio(); !c.esFin(); c.siguiente()){
            ListaConPI<BuscadorDeLaBibl.Posting> valor = index.recuperar(c.recuperar());
            if (valor.talla() == 1) res.insertar(c.recuperar());
        }
        if (res.talla() == 0) res = null;
        return res;
    }
    
    /**
     * Se pide añadir un nuevo método público a la clase BuscadorDeLaBibl con el siguiente 
     * perfil:
     * public ListaConPI<Termino> comienzanPor(String prefijo, String libro)
     *
     * El método recibe dos String prefijo y libro, y devuelve una ListaConPI de términos 
     * de la BD que comiencen por la String prefijo dentro del libro cuyo título se indica 
     * en la String  libro.
     */
    public ListaConPI<Termino> comienzanPor(String prefijo, String libro){
        ListaConPI<Termino> list = new LEGListaConPI<Termino>();
        ListaConPI<Termino> c = index.claves();
        if (!esTermino(prefijo)) return null;
        for(c.inicio();!c.esFin();c.siguiente()){
            Termino t = c.recuperar();
            String term = t.toString();
            if (term.startsWith(prefijo)){
                if (list.recuperar() != null && list.recuperar() != t ) list.insertar(c.recuperar());
            }
            
            
            //if (list.recuperar() != t)
            //Hacer que no salgan repetidos
    ...
    public ListaConPI<Termino> comienzanPor(String prefijo, String libro){
        ...
        for(c.inicio();!c.esFin();c.siguiente()){
            Termino t = c.recuperar();
            String term = t.toString();
            if (term.startsWith(prefijo)) list.insertar(c.recuperar());
            //Hacer que no salgan repetidos
